...
local function CreatePlayerStyle(self, unit, isSingle)
	...
	lib.ThreatBar(self)
	if cfg.Portrait then lib.gen_portrait(self) end
	if cfg.Runebar then lib.genRunes(self) end
	if cfg.Holybar then lib.genHolyPower(self) end
	if cfg.WarlockSpecBars then lib.genShards(self) end
	if cfg.Eclipsebar then lib.addEclipseBar(self) end
	if cfg.ShadowOrbsBar then lib.ShadowOrbsBar(self) end
end
