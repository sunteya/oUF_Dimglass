...
lib.genShards = function(self)
	...
	local wb = CreateFrame("Frame", nil, UIParent)

	if cfg.HealMode then
		wb:SetFrameStrata("MEDIUM")
		wb:SetFrameLevel(12)
		wb:SetPoint("BOTTOMLEFT", self, "TOP", -108,-5)
		wb:SetSize(90, 3)
		CreateBorder(wb,1)
	else
		wb:SetFrameStrata("LOW")
		wb:SetPoint("BOTTOM", UIParent, "BOTTOM", 0,200)
		wb:SetSize(90, 6)
		CreateBorder(wb,1)
		CreateShadow(wb,5)
	end
	...
	for i = 1, 4 do					
		...
		--wb[i]:SetStatusBarColor(148/255, 130/255, 201/255)
		wb[i]:SetStatusBarColor(160/255, 32/255, 240/255)
		...
		wb[i]:SetWidth((wb:GetWidth() - 2)/3)
		wb[i].bg:SetTexture(cfg.statusbar_texture)					
		wb[i].bg:SetAlpha(.15)
	end

	if cfg.HealMode ~= true then
		local wbText = UIParent:CreateFontString(nil, 'OVERLAY')
		wbText:SetPoint("CENTER", wb, "CENTER", 0, 0)	
		wbText:SetFont(cfg.font, 9, "OUTLINE")
		--wbText:SetFrameLevel("20")
		wb:SetScript("OnEvent",function()
			local spec = GetSpecialization()
			if spec then
				if spec == SPEC_WARLOCK_DEMONOLOGY then
					wbText:SetText(format("%s / %s",UnitPower("player", SPELL_POWER_DEMONIC_FURY),UnitPowerMax("player", SPELL_POWER_DEMONIC_FURY)))
				else
					wbText:SetText( )
				end
			end
		...
		wb:RegisterEvent("UNIT_POWER")
		wb:RegisterEvent("UNIT_MAXPOWER")
	end
...
lib.genHolyPower = function(self)
	...
	if cfg.HealMode then
		...
		bars:SetPoint("BOTTOMLEFT", self, "TOP", -107,-5)
		bars:SetSize(75, 3)
		CreateBorder(bars,1)
	else
...
lib.genRunes = function(self)
	...
	if cfg.HealMode then
		...
		runes:SetPoint("BOTTOM", self, "TOP", 0,-5)
		runes:SetSize(self:GetWidth()-5, 3)
